...
def generate_harmonics(times, data, **kwargs):
    ...
    for i in range(0, num_harmonics):
        ...
        freq_idx=np.where((frequencies<(true_harm+(input_frequency*kwargs["filter_val"]))) & (frequencies>true_harm-(input_frequency*kwargs["filter_val"])))
        ...
        if kwargs["zero_shift"]==True:
            ...
            for j in [-1,1]:
                ...
                top_hat[np.where((frequencies>(j*true_harm+(input_frequency*kwargs["filter_val"]))) | (frequencies<true_harm*j-(input_frequency*kwargs["filter_val"])))]=0
...
def inv_objective_fun(times, time_series,**kwargs):
    ...
    if func==None or func_not_right_length==True:
        ...
        first_harm=(kwargs["harmonics"][0]*input_frequency)-(input_frequency*0.5)
        last_harm=(kwargs["harmonics"][-1]*input_frequency)+(input_frequency*0.5)
...
def plot_harmonics(**kwargs):
    ...
    for label in label_list:
        ...
        if kwargs["xaxis"]=="DC_potential":
            ...
            fft_freq=np.fft.fftfreq(len(pot), time_series_dict[label]["time"][1]-time_series_dict[label]["time"][0])
            max_freq=sci.get_frequency(time_series_dict[label]["time"],time_series_dict[label]["current"])
        ...
        if "harmonics" not in kwargs:
            time_series_dict[label]["harmonics"]=sci.maximum_availiable_harmonics(time_series_dict[label]["time"],time_series_dict[label]["current"])
        ...
        harm_dict[label]=sci.plot.generate_harmonics(time_series_dict[label]["time"], time_series_dict[label]["current"], 
    ...
    if "axes_list" not in kwargs:
        fig,kwargs["axes_list"]=plt.subplots(num_harmonics, 1)    
            ...
            raise ValueError("Wrong number of axes for harmonics (calculated number of harmonics is {0}, provided axes is of length {1})".format(num_harmonics, len(kwargs["axes_list"])))
    ...
    for plot_name in label_list:
        ...
        for i in range(0, len(harmonics)):
            ...
            if i==0:
                ...
                ax.plot(xaxis, pf(harm_dict[plot_name][i,:]), label=plot_name, alpha=kwargs["alpha"], color=kwargs["colour"], lw=kwargs["lw"])
            else:
                ax.plot(xaxis, kwargs["plot_func"](harm_dict[plot_name][i,:]), alpha=kwargs["alpha"], color=kwargs["colour"], lw=kwargs["lw"])
