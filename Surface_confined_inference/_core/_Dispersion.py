...
class Dispersion:
    def __init__(self, dispersion_bins, dispersion_parameters, dispersion_distributions, optim_list, fixed_parameters={}):
        ...
        if len(self.distributions)!=len(self.dispersion_parameters):
            print("Dipersion distributions are " ,self.distributions,"dispersion parameters are ",self.dispersion_parameters)
    ...
    def generic_dispersion(self, dim_dict, GH_dict=None):
        ...
        for i in range(0, len(self.dispersion_parameters)):
            if self.distributions[i]=="uniform":
                    value_arrays.append(np.linspace(self.dispersion_parameters[i]+"_lower", self.dispersion_parameters[i]+"_upper", self.bins[i]))
