...
def check_input_dict(input_dict, parameters):
    ...
    if len(missing)>0:
        raise ValueError("Simulation requires the following parameters: {0}".format((" ").join(list(missing))))
    if len(extra)>0:
        raise ValueError("The following parameters are not required for the simulation: {0}".format((" ").join(list(extra))))
...
def maximum_availiable_harmonics(times, current):
    ...
    while in_noise==False:
        ...
        noise_level=np.mean(fft[np.where((frequencies>(index-0.5*input_freq)) & (frequencies<(index-0.4*input_freq)))])
        peak_level=max(fft[np.where((frequencies>(index-0.1*input_freq)) & (frequencies<(index+0.1*input_freq)))])
